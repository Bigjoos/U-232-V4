...
{
			Csql_result result = Csql_query(m_database, "select info_hash, @fid, freetorrent from @files where flags & 2").execute();
			while (Csql_row row = result.fetch_row())
			{
				auto i = m_torrents.find(to_array<char, 20>(row[0]));
				if (i != m_torrents.end())
				...
				Csql_query(m_database, "update @files set flags = flags & ~2 where @fid = ?")(row[1].i()).execute();
			}
		}
...
void write_db_torrents()
{
	...
	if (!m_announce_log_buffer.empty())
	{
		...
		async_query("insert into " + db_name("announce_log") + " (ipa, port, event, info_hash, peer_id, downloaded, left0, uploaded, uid, mtime) values " + m_announce_log_buffer);
		m_announce_log_buffer.erase();
	}
	if (!m_scrape_log_buffer.empty())
	{
		m_scrape_log_buffer.pop_back();
		async_query("insert into " + db_name("scrape_log") + " (ipa, uid, mtime) values " + m_scrape_log_buffer);
...
void write_db_users()
{
	...
	if (!m_torrents_users_updates_buffer.empty())
	{
		...
		async_query("insert into " + db_name("files_users") + " (active, announced, completed, completedtime, peer_id, upspeed, downspeed, leechtime, seedtime, ipa, downloaded, `left`, uploaded, mtime, started, fid, uid) values "
...
std::string srv_insert_peer(const Ctracker_input& v, bool udp, t_user* user)
{
	...
	if (m_use_sql && user && file.fid)
	{
			...
			&& v.m_uploaded >= i->uploaded)
		{
			...
			if (file.freetorrent == 1) 
      			{
                		downloaded = 0;
                		uploaded = v.m_uploaded - i->uploaded;
            		}
            		else if (file.freetorrent == 2) 
            		{
                		downloaded = 0;
                		uploaded = 0;
            		} 
            		...
            		all_downloaded = v.m_downloaded - i->downloaded;
            		all_uploaded = v.m_uploaded - i->uploaded;
            
			timespent = srv_time() - i->mtime;
			if (v.m_left || v.m_event == Ctracker_input::e_completed)
                                        leechtime = srv_time() - i->mtime;
                                else
                                        seedtime = srv_time() - i->mtime;
		...
		m_torrents_users_updates_buffer += Csql_query(m_database, "(?,1,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?),")
			...
			(v.m_event == Ctracker_input::e_completed ? srv_time() : 0)
            		(v.m_peer_id)
			(upspeed)
		    	(downspeed)
		    	(leechtime)
