...
$res = sql_query('SELECT t.id,t.name,t.descr,t.size,t.category,t.seeders,t.leechers,t.added, c.name as catname FROM torrents as t LEFT JOIN categories as c ON t.category = c.id WHERE ' . $where . ' t.visible="yes" AND t.checked_by IS NOT NULL ORDER BY t.added DESC ' . $limit) or sqlerr(__FILE__, __LINE__);
