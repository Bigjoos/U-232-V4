...
//== getuserfrompasskey
/*function get_user_from_passkey($passkey)
{
    global $mc1, $INSTALLER09;
    if (strlen($passkey) != 32 || !bin2hex($passkey)) return false;
    ...
    if (($user = $mc1->get_value($key)) === false) {
        ...
        $user_query = ann_sql_query("SELECT " . $user_fields . " FROM users WHERE passkey=" . ann_sqlesc($passkey) . " OR torrent_pass=" . ann_sqlesc($passkey) . " AND enabled = 'yes'") or ann_sqlerr(__FILE__, __LINE__);
