...
if (bencdec::get_type($info['name']) != 'string' || bencdec::get_type($info['piece length']) != 'integer' || bencdec::get_type($info['pieces']) != 'string') stderr('Error', 'invalid torrent, invalid types in info dictionary');
$dname = $info['name'];
...
if ($num_pieces != $expected_pieces) stderr('Whoops', 'total file size and number of pieces do not match');
//==
